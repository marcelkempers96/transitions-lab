§ / Programmes

# Energy & Water Systems

*How energy and water systems are built, paid for and relied on, where the network reaches and where it never did.*

<div class="research-areas-panel">
  <span class="ra-label">Research areas</span>
  <ul>
    <li><a href="#ra-mini-grids">Mini-grids and off-grid supply</a></li>
    <li><a href="#ra-grid">Renewables, storage and grid integration</a></li>
    <li><a href="#ra-water">Water delivery, vendors and payment</a></li>
  </ul>
</div>

<div class="sdg-block">
  <span class="sdg-label">Contributes to</span>
  <ul class="sdg-row">
    <li class="sdg-07"><a href="/sdgs#g7"><img class="sdg-icon" src="/assets/img/sdg/sdg-07.svg" alt="SDG 7 Affordable and Clean Energy"><span class="sdg-name">Reaching the last mile fairly</span></a></li>
    <li class="sdg-06"><a href="/sdgs#g6"><img class="sdg-icon" src="/assets/img/sdg/sdg-06.svg" alt="SDG 6 Clean Water and Sanitation"><span class="sdg-name">Water access that is trusted</span></a></li>
    <li class="sdg-11"><a href="/sdgs#g11"><img class="sdg-icon" src="/assets/img/sdg/sdg-11.svg" alt="SDG 11 Sustainable Cities and Communities"><span class="sdg-name">Grid integration on the ground</span></a></li>
  </ul>
</div>

<!-- IMAGE energy-hero: hero photo | Solar mini-grid, home solar panel, water point or piped connection in daily use. Real, working, daylight. | Full-width hero under the standfirst. -->

Energy and water are the two systems everything else in a transition rests on. They are also the two where the same research question keeps arriving in different clothes: how does a metered, paid-for service reach a household the network was not built for, and stay reliable enough that people plan around it.

## Inside the programme

<span class="ra-name" id="ra-mini-grids">Mini-grids and off-grid supply</span> is the research area closest to the Lab's earliest energy fieldwork. A pay-as-you-go connection is a mini-utility with a repayment schedule, and its success depends on affordability at a household's real income, reliability that people plan against, and after-sales that outlives the founder. The Lab looks at what actually keeps a household paying, and who sits at the very end of the access curve when everybody else has moved on.

<span class="ra-name" id="ra-grid">Renewables, storage and grid integration</span> is where the same programme meets the grid that already exists. Curtailment, storage siting, coincident peak, and the anchor-tenant contracts that decide who firms the system: none of these are engineering questions in the strict sense. Each is a decision about who bears the cost of a shared piece of infrastructure, and the Lab reads them through the utility, the developer, the industrial buyer, and the residential customer at once.

<span class="ra-name" id="ra-water">Water delivery, vendors and payment</span> is the other half of the same argument, at a different tap. Kiosks, meters, vending points and household connections are all payment interfaces layered over a delivery system that has to be trusted before it is used. The Lab studies transboundary and drought-stressed catchments alongside the peri-urban vendor markets where the affordability question is at its sharpest.

Water is where the Lab's roots run deepest: based in Delft, a global centre of water research, we pair genuine technical understanding of water systems with the field methods to study how they actually serve people.

---

## The market, in brief

**Energy access.**

- Sub-Saharan Africa accounted for [**563 million**](https://www.iea.org/reports/financing-electricity-access-in-africa) of the 655 million people without electricity in 2024 (IEA), nearly **80% rural**.
- Reaching universal access needs about [**$15 billion**](https://www.iea.org/reports/financing-electricity-access-in-africa) a year; under [**$2.5 billion**](https://www.iea.org/reports/financing-electricity-access-in-africa) was committed in 2023 (IEA).
- Mini-grids and standalone solar are among the [**most cost-effective**](https://trackingsdg7.esmap.org/) ways to close the rural gap (Tracking SDG7, ESMAP).

**Water and sanitation.**

- Globally, [**1 in 4 people**](https://www.who.int/news/item/26-08-2025-1-in-4-people-globally-still-lack-access-to-safe-drinking-water---who--unicef) (2.1 billion) still lack safely managed drinking water; [**3.4 billion**](https://www.who.int/news/item/26-08-2025-1-in-4-people-globally-still-lack-access-to-safe-drinking-water---who--unicef) lack safely managed sanitation (WHO/UNICEF JMP, 2025).
- In sub-Saharan Africa, rural access sits at just [**53%**](https://washdata.org/) (WHO/UNICEF JMP), and the region is the **only one** where the absolute number without access has grown.
- Women and girls bear the collection burden, many spending [**over 30 minutes**](https://www.who.int/teams/environment-climate-change-and-health/water-sanitation-and-health) a day fetching water (WHO).

---

## What we see that others miss

**Energy.** The technology is largely settled. Whether a household connects, keeps paying, and benefits depends on affordability, reliability, and whether the system was designed around how people actually use power.

The Lab documents that. We are the party asking whether locally-owned developers can scale, whether access converts into the productive uses that make it self-sustaining, and **who sits at the very end of the access curve**, the finding that decides if a transition is fair. Our [Pyropower case study](/case-pyropower) shows waste-to-energy at the smallholder scale.

**Water.** The technical solutions mostly exist. Success depends on whether infrastructure is sustained, trusted, and maintained, and whether it reaches the rural and marginalised.

The Lab documents why a hand pump is maintained in one village and abandoned in the next, what builds or erodes trust in a source, and whether sanitation behaviour actually changes beyond the counting of latrines. That is the socio-technical layer an engineering audit never reaches, and the one that decides whether investment lasts.

---

## Knowledge we draw on

- IEA, [Financing Electricity Access in Africa](https://www.iea.org/reports/financing-electricity-access-in-africa).
- World Bank & ESMAP, [Tracking SDG7](https://trackingsdg7.esmap.org/) and [Mission 300](https://www.worldbank.org/en/news/press-release/2025/06/25/energy-access-has-improved-yet-international-financial-support-still-needed-to-boost-progress-and-address-disparities).
- ESMAP, [Off-Grid Solar Market Trends](https://www.esmap.org/).
- WHO/UNICEF JMP, [Progress on Household Drinking Water and Sanitation](https://washdata.org/) and the [2025 report](https://www.who.int/news/item/26-08-2025-1-in-4-people-globally-still-lack-access-to-safe-drinking-water---who--unicef).
- World Bank, [water in Eastern and Southern Africa](https://www.worldbank.org/en/region/afr/brief/afe-water).
- WHO, [global WASH monitoring](https://www.who.int/teams/environment-climate-change-and-health/water-sanitation-and-health).
- Our own [Pyropower case study](/case-pyropower) and [MiMaji water transparency case study](/case-mimaji).

---

## Related reading

<div class="reading-group">
  <h3>Mini-grids and off-grid supply</h3>
  <div class="reading-cards">
    <a href="/insight-stacking-not-switching">
      <h4>Stacking, Not Switching</h4>
      <p>How households actually build up an energy portfolio, one fuel at a time.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
    <a href="/insight-nobody-buys-a-chiller">
      <h4>Nobody Buys a Chiller</h4>
      <p>The counterfactual at the centre of efficiency-as-a-service.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
    <a href="/insight-who-pays-decides">
      <h4>Who Pays Decides What Gets Built</h4>
      <p>The hyperscaler class now setting the direction of firm clean generation.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
  </div>
</div>

<div class="reading-group">
  <h3>Renewables, storage and grid integration</h3>
  <div class="reading-cards">
    <a href="/insight-paying-for-what-we-curtail">
      <h4>Paying for Power You Curtail</h4>
      <p>Why clean generation and cheap electricity are not the same thing.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
    <a href="/insight-customers-who-can-leave">
      <h4>The Customers Who Can Leave</h4>
      <p>The defection dynamic in shared infrastructure, and who is left carrying the fixed cost.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
    <a href="/insight-load-that-grows-when-hot">
      <h4>The Load That Grows When It Is Hot</h4>
      <p>Cooling-driven data centre demand is not baseload, and grid planning treats it as if it were.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
  </div>
</div>

<div class="reading-group">
  <h3>Water delivery, vendors and payment</h3>
  <div class="reading-cards">
    <a href="/insight-what-the-bond-secures">
      <h4>What the Bond Is Actually Secured On</h4>
      <p>Why the securitisation move does not transfer from solar to water.</p>
      <span class="read-cue">Read &rarr;</span>
    </a>
  </div>
</div>

For the full series across the transitions we study, see [Articles](/articles).

---

<div class="programme-footer">
  <h3>Where to next</h3>
  <div class="footer-links">
    <a href="/case-pyropower">Pyropower case</a>
    <a href="/case-mimaji">MiMaji water case</a>
    <a href="/field-research">Field Research method</a>
    <a href="/expertise-ai-digital">AI &amp; Digital Systems</a>
    <a href="/expertise-finance">Financial Inclusion</a>
    <a href="/economics-of-transitions">Economics of Transitions</a>
    <a href="/sdgs">Research by SDG</a>
    <a href="/articles">All articles</a>
  </div>
</div>

<div class="contact-cta-programme">
  <h3>Independent evidence on energy and water systems.</h3>
  <p class="lede">If you are building, financing or regulating a network reaching households the grid was not built for, tell us the decision you are facing.</p>
  <div class="contact-options">
    <a href="/entering-a-new-context"><strong>Enter a new context</strong><span>Market entry &amp; adoption</span></a>
    <a href="/measuring-change"><strong>Measure change</strong><span>Reach, depth, outcomes</span></a>
    <a href="/reporting-to-funders"><strong>Report to funders</strong><span>Baseline through endline</span></a>
  </div>
  <div class="cta-actions">
    <a href="/contact" class="cta-primary">Start a conversation &rarr;</a>
    <a href="/what-we-do" class="cta-secondary">See how we work</a>
  </div>
</div>
