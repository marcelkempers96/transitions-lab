...
## What an 84% monthly fall actually measures
...
This is not pedantry about venture data, which nobody needs to care about deeply. It is the identical error that produces unusable impact claims: a large percentage computed on a small or volatile base, reported without the base, and then interpreted as a change in behaviour. We flagged the same pattern in the [European monitoring literature](/esf-social-innovation). It is the most common failure in the field and it is entirely avoidable.

---

## The claim underneath, which is more interesting and also a problem
...
- **Trailing twelve-month volume and deal count, split by sector.** The only version of the headline that means anything.
- **Median round size, not mean.** The mean is a mega-round detector. The median tells you what is happening to ordinary companies.
- **Time between rounds.** Lengthening intervals are the clearest early signal of a tightening market, and they lengthen before volumes fall.
- **Share of rounds with a development finance institution, a corporate strategic, or a debt provider on the cap table.** If capital is genuinely becoming infrastructure-oriented, the composition of who is investing changes before the totals do. This is the specific number that would confirm or kill the thesis.
- **Survival and revenue at 24 months.** The outcome everything else is a proxy for.

---

## Why this belongs on a research organisation's site
...
We hold ourselves to that when we report on programmes, which is what [independent measurement](/impact-measurement) means in practice, and it is the reason our [readiness assessments](/readiness-levels) tend to produce less exciting numbers than the ones they replace. The same scepticism applies to the claim that AI is straightforwardly good for the energy transition, which we took apart in [The Asymmetry Nobody Is Metering](/insight-ai-absorptive-capacity), and to the assumption that European invention converts automatically into deployment, which it [does not](/insight-eu-us).

If you are a funder being handed a number and you want to know whether it means what it appears to mean, [that is a question we answer](/contact). Often the honest answer is that the number cannot bear the weight being put on it, which is worth knowing before it goes into a board paper.
